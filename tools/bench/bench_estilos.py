...
TIPOS = RAIZ / "src" / "Modux" / "shared" / "Types"
SELFOF = TIPOS / "SelfOf.luau"
...
def build_v3real(d: pathlib.Path, n: int):
    ...
    base(d)
    shutil.copy(SELFOF, d / "SelfOf.luau")
    shutil.copy(TIPOS / "Pick.luau", d / "Pick.luau")
...
def build_v2sites(d: pathlib.Path, n: int):
    """v2, mas com N sitios de declaracao — o par honesto do v3sites.

    No V2 o `Modux.Controller(id)` devolve `any`: o tipo nao chega pela
    chamada, chega por um alias que o Watcher escreve. Entao o sitio tipado do
    V2 e `local C: T.ModX = M.Controller("ModX")` — um alias simples, resolvido
    uma vez, sem type function e sem recortar o Manifest.

    E essa a diferenca que a medicao isola: os dois tem N sitios, os dois
    resolvem um indice central, e so um deles roda uma VM para fazer isso.
    """
    build_v2(d, n)
    for i in range(n):
        escrever(d, f"Mod{i}.luau", "\n".join([
            ...
            'local M = require("./Framework")',
            'local T = require("./Types")',
            f'local C: T.Mod{i} = M.Controller("Mod{i}")',
            "C:OnInit(function(self)",
            "\tself:DoWork(1)",
            "end)",
            "return C",
        ]) + "\n")
    # o Framework do v2 fixava o retorno em T.Mod0; com N sitios cada um pede
    # o seu, entao ele volta a ser `any` — que e o que o V2 devolve de verdade.
    fw = (d / "Framework.luau").read_text(encoding="utf-8")
    fw = fw.replace("function M.Controller(id: string): T.Mod0",
                    "function M.Controller(id: string): any")
    (d / "Framework.luau").write_text(fw, encoding="utf-8")
    # o Body e o arquivo em que a completion e medida, entao ele tem que ter a
    # mesma forma dos outros: sem a anotacao a sonda cairia num `any`.
    escrever(d, "Body.luau", "\n".join([
        "--!strict",
        'local M = require("./Framework")',
        'local T = require("./Types")',
        'local C: T.Mod0 = M.Controller("Mod0")',
        "C:OnInit(function(self)",
        "\tself:DoWork(1)",
        "end)",
        "return C",
...
SONDA = {
    ...
    "puro-plano": ("\tMod1.D", 4),
    "v2": ("\tself.D", 4),
    "v2sites": ("\tself.D", 5),   # o require do Types empurra uma linha
    "v3ideia": ("\tself.D", 4),
    "v3real": ("\tself.D", 4),
    "v3sites": ("\tself.D", 4),
    "v3pre": ("\tself.D", 4),
...
# Quantas vezes cada analyze roda. A mediana de REPS e o que vai para a tabela:
# uma rodada so pega qualquer hiccup de disco ou de agendador e vira "numero".
REPS = int(os.environ.get("BENCH_REPS", "3"))


def analisar(d: pathlib.Path, reps=None) -> tuple[float, int, int]:
    # nome relativo, nao caminho absoluto: o subprocess ja roda com cwd=d, e a
    # linha de comando do Windows para em 32 KB. Com caminho absoluto o N=200
    # estourava o limite e o erro que chegava era
    # `FileNotFoundError: [WinError 206] The filename or extension is too long`,
    # que nao parece ter nada a ver com o tamanho do projeto.
    arquivos = [p.name for p in d.glob("*.luau")]
...
def linha_de_base() -> float:
    ...
    ms, _, _ = analisar(d, reps=max(REPS, 5))
...
def main():
    ...
    # BENCH_ESTILOS=v2,v3sites roda so esses. Para o N grande nao faz sentido
    # pagar as onze variantes: as que interessam sao a linha de base, o V2 e o
    # desenho atual.
    filtro = [x for x in os.environ.get("BENCH_ESTILOS", "").split(",") if x]
    if filtro:
        faltando = [x for x in filtro if x not in ESTILOS]
        if faltando:
            raise SystemExit(f"estilo desconhecido: {faltando}; use {list(ESTILOS)}")
    defs = definitions()
